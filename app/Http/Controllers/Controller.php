    ...
    protected function form_users($result, $table, $id, $subtype)
    {
        ...
        if ($id == '0') {
            ...
            if ($subtype == '2') {
                ...
                $data2 = [
                    'specialty' => isset($provider->specialty)?$provider->specialty:'',
                    'license' => isset($provider->license)?$provider->license:'',
                    'license_state' => isset($provider->license_state)?$provider->license_state:'',
                    'npi' => isset($provider->npi)?$provider->npi:'',
                    'npi_taxonomy' => isset($provider->npi_taxonomy)?$provider->npi_taxonomy:'',
                    'upin' => isset($provider->upin)?$provider->upin:'',
                    'dea' => isset($provider->dea)?$provider->dea:'',
                    'medicare' => isset($provider->medicare)?$provider->medicare:'',
                    'tax_id' => isset($provider->tax_id)?$provider->tax_id:'',
                    'rcopia_username' => isset($provider->rcopia_username)?$provider->rcopia_username:'',
                    'schedule_increment' => isset($provider->schedule_increment)?$provider->schedule_increment:'',
                    'peacehealth_id' => isset($provider->peacehealth_id)?$provider->peacehealth_id:'',
                    'practice_id' => isset($provider->practice_i)?$provider->practice_i:''
                ];
            }
        }
        if ($id !== '0') {
    ...
    protected function generate_pdf($html, $filepath, $footer='footerpdf', $header='', $type='1', $headerparam='', $watermark='')
    {
        ...
        PDF::setFooterCallback(function($pdf) use ($footer) {
            ...
            if ($footer == 'footerpdf') {
                ...
                $footer_html .= '<p style="text-align:center; font-size: 8px;">This document was generated by AKRA TELEHEALTH</p>';
            }
        ...
        PDF::SetAuthor('AKRA TELEHEALTH');
        PDF::SetTitle('AKRA TELEHEALTH PDF Document');
        if ($type == '1') {
