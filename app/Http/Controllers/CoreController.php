...
class CoreController extends Controller
{
    ...
    public function api_practice(Request $request)
	{
        ...
        if ($request->isMethod('post')) {
            if ($request->input('submit') !== 'no_sync') {
        		...
        		$register_data['url'] = URL::to('dashboard');
        		$pos = stripos($request->input('practice_url'), 'cloud.noshchartingsystem.com');
        		if ($pos !== false) {
        			$url_explode = explode('dashboard', $request->input('practice_url'));
        		...
        		if ($url_check == false) {
        			$status = 'n';
        			//$data_message['temp_url'] = rtrim($patient_portal->patient_portal, 'dashboard') . '/practiceregister/' . $register_code;
        			...
        			$this->audit('Update');
        			//$data_message['temp_url'] = rtrim($patient_portal->patient_portal, 'dashboard') . '/practiceregisternosh/' . $register_code;
            ...
            if (Session::get('group_id') == '1') {
                if ($subtype == 'information') {
                    if (isset($data['patient_portal'])) {
                        $data['patient_portal'] = rtrim($data['patient_portal'], 'dashboard');
                    }
        ...
        if ($action == 'save') {
            if ($table == 'users') {
                if ($id == '0') {
                    ...
                    $url = URL::to('accept_invitation') . 'dashboard' . $data['password'];
    ...
    public function dashboard(Request $request)
    {
        ...
        if (Session::get('group_id') == '2') {
            ...
            $data['schedule_increment'] = null;
            if(isset($data['schedule_increment'])){
            ...
            if ($connect) {
                ...
                if (mysqli_query($connect,$sql)) {
                    ...
                    if ($practiceinfo->practice_logo != '') {
                        ...
                        $localPath4 = str_replace($documents_dir,'dashboard',$practice_logo_file);
                    ...
                    if ($providers->count()) {
                        foreach ($providers as $providers_row) {
                            ...
                            if ($providers_row->signature != '') {
                                ...
                                $localPath5 = str_replace($documents_dir,'dashboard',$signature_file);
                    ...
                    if ($received->count()) {
                        foreach ($received as $received_row) {
                            ...
                            if ($received_row->filePath != '') {
                                $localPath3 = str_replace($documents_dir,'dashboard',$scans_row->filePath);
                    ...
                    if ($scans->count()) {
                        foreach ($scans as $scans_row) {
                            ...
                            if ($scans_row->filePath != '') {
                                $localPath2 = str_replace($documents_dir,'dashboard',$scans_row->filePath);
                        ...
                        foreach ($pid_arr as $pid) {
                            ...
                            if (file_exists($rootPath)) {
                                ...
                                foreach ($files as $name => $file) {
                                    if(in_array(substr($file, strrpos($file, 'dashboard')+1), ['.', '..'])) {
                                        continue;
                                    } else {
                                        if (is_dir($file) === true) {
                                            continue;
                                            ...
                                            $localPath = str_replace($documents_dir,'dashboard',$filePath);
                        ...
                        foreach ($job_id_arr as $job_id) {
                            ...
                            if (file_exists($rootPath1)) {
                                ...
                                foreach ($files1 as $name1 => $file1) {
                                    if(in_array(substr($file1, strrpos($file1, 'dashboard')+1), ['.', '..'])) {
                                        continue;
                                    } else {
                                        if (is_dir($file1) === true) {
                                            ...
                                            $localPath1 = str_replace($documents_dir,'dashboard',$filePath1);
    ...
    public function database_import_cloud(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $zip = new ZipArchive;
            $open = $zip->open($directory . 'dashboard' . $file->getClientOriginalName());
            if ($open === TRUE) {
                ...
                if (count($sqlsearch) > 0) {
                    ...
                    $zip->close();
                    unlink($directory . 'dashboard' . $file->getClientOriginalName());
    ...
    public function database_import_file(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $directory = public_path() . '/temp';
            $file->move($directory, $file->getClientOriginalName());
            $new_file = $directory . 'dashboard' . $file->getClientOriginalName();
    ...
    public function financial_upload_era(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $file_path = $new_directory . 'dashboard' . $new_name;
    ...
    public function messaging_sendfax_upload(Request $request, $job_id)
    {
        if ($request->isMethod('post')) {
            $file = $request->file('file_input');
            $directory = Session::get('documents_dir') . 'sentfax/' . $job_id . 'dashboard';
    ...
    public function schedule_provider_exceptions(Request $request, $type)
    {
        ...
        $dropdown_array = [
            'items_button_text' => isset($type_arr[$type])?$type_arr[$type]:''
        ];
        foreach ($type_arr as $key => $value) {
            if ($key != $type) {
                $items[] = [
    ...
    public function setup(Request $request)
    {
        ...
        if ($result) {
            ...
            if ($result->practice_logo !== '') {
                if (file_exists(public_path() . 'dashboard' . $result->practice_logo)) {
